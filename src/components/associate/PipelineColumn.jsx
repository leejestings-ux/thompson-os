...
export default function PipelineColumn({ state, donors }) {
  ...
  return (
    <div className="flex-shrink-0 w-72 flex flex-col bg-table-stripe/50 rounded-xl border border-border/50 h-full">
      {/* Column header */}
      <div className="px-3.5 py-3 border-b border-border/50 flex items-center gap-2.5 flex-shrink-0">
        <span className={`w-2 h-2 rounded-full ${dotColor}`} />
        <h3 className="text-xs font-semibold text-charcoal uppercase tracking-wider flex-1">
          {state}
        </h3>
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2.5">
        {donors.length === 0 ? (
          <p className="text-xs text-muted text-center py-6">No donors</p>
        ) : (
          donors.map((donor) => (
